...
def index():
    if request.method == "POST" and current_user.is_authenticated:  # ←vscordが読みきれてないためにエラーが出てるだけなので気にしなくていい
...
def show(message_id):
    message = Message.get_by_id(message_id)  # 講義内で追加
    messages = (
        Message.select()
        # .where((Message.id == message_id) | (Message.reply_to == message_id))
        .where((Message.reply_to == message_id)).order_by(Message.pub_date.desc(), Message.id.desc())
    )

    return render_template("show.html", message=message, messages=messages, message_id=message_id)
